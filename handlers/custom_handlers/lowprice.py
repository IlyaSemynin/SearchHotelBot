...
def low_price(message: Message) -> None:
    ...
    :return: None
    """
    bot.set_state(message.from_user.id, LowPriceInfoState.wait_city, message.chat.id)
...
@bot.message_handler(state=LowPriceInfoState.wait_city)
def low_wait_city(message: Message) -> None:
    """
    Уточняем у пользователя при помощи клавиатуры в каком именно городе он хочет найти отель.
    :param message: Message
    :return: None
    """
    bot.set_state(message.from_user.id, LowPriceInfoState.city, message.chat.id)
    try:
        bot.send_message(message.from_user.id, "Уточните название города(low)", reply_markup=city_markup(message.text))
    except:
...
def low_get_city(call) -> None:
    ...
    if call.message:
        ...
        low_choice_currency(call)


def low_choice_currency(call) -> None:
    """
    ...
    bot.set_state(call.from_user.id, LowPriceInfoState.currency_selection, call.message.chat.id)


@bot.message_handler(state=LowPriceInfoState.currency_selection)
def low_get_night_price(message: Message) -> None:
    """
...
@bot.message_handler(state=LowPriceInfoState.count)
def low_get_count(message: Message) -> None:
    """
    ...
    bot.set_state(message.from_user.id, LowPriceInfoState.photo_count, message.chat.id)
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data["count"] = message.text
        logger.info(f"Выбрано кол-во отелей: {data['count']}")


@bot.message_handler(state=LowPriceInfoState.photo_count)
def low_get_photo_count(message: Message) -> None:
    """
    ...
    if message.text.lower() == 'да':
        ...
        bot.set_state(message.from_user.id, LowPriceInfoState.photo, message.chat.id)
        logger.info("Фотографии отеля? Да!")
    elif message.text.lower() == 'нет':
        bot.set_state(message.from_user.id, LowPriceInfoState.photo, message.chat.id)
        low_get_photo(message)
        logger.info("Фотографии отеля? Нет!")


@bot.message_handler(state=LowPriceInfoState.photo)
def low_get_photo(message: Message) -> None:
    """
