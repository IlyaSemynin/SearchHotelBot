...
def high_wait_city(message: Message) -> None:
    ...
    try:
        bot.send_message(message.from_user.id, "Уточните название города(high)", reply_markup=city_markup(message.text))
    except:
...
def high_choice_currency(call) -> None:
    ...
    bot.set_state(call.from_user.id, HighPriceInfoState.currency_selection, call.message.chat.id)
